...
import {AtLoadingContext} from './at-loading.directive';
import {AtLoadingComponent, LoadingType, LoadingMode, LoadingStrategy, LoadingStyle} from './at-loading.component';
import {IAtLoadingConfig} from './at-loading.service';

export interface IInternalLoadingOptions extends IAtLoadingConfig {
    height?: number;
    style?: LoadingStyle;
}
...
export class AtLoadingFactory {
     ...
     */
    public createFullScreenComponent(options: IAtLoadingConfig): ILoadingRef {
        ...
        loadingRef.observable
            .subscribe((registered: number) => {
                if (registered > 0 && !loading) {
                    ...
                    loadingRef.componentRef = overlayRef.attach(new ComponentPortal(AtLoadingComponent));
     ...
     * Saves a reference in context to be called when registering/resolving the loading element.
     */
    public createOverlayComponent(options: IAtLoadingConfig, viewContainerRef: ViewContainerRef,
     ...
     * Saves a reference in context to be called when registering/resolving the loading element.
     */
    public createReplaceComponent(options: IAtLoadingConfig, viewContainerRef: ViewContainerRef,
    ...
    private _createComponent(options: IInternalLoadingOptions): ILoadingRef {
        ...
        compRef.componentRef = this._componentFactoryResolver
            .resolveComponentFactory(AtLoadingComponent).create(this._injector);
     ...
     */
    private _mapOptions(options: IInternalLoadingOptions, instance: AtLoadingComponent): void {
...
export function LOADING_FACTORY_PROVIDER_FACTORY(parent: AtLoadingFactory, componentFactoryResolver: ComponentFactoryResolver, overlay: Overlay, injector: Injector): AtLoadingFactory {
    return parent || new AtLoadingFactory(componentFactoryResolver, overlay, injector);
}

export const LOADING_FACTORY_PROVIDER: Provider = {
    // If there is already a service available, use that. Otherwise, provide a new one.
    provide: AtLoadingFactory,
    deps: [[new Optional(), new SkipSelf(), AtLoadingFactory], ComponentFactoryResolver, Overlay, Injector],
